...
contract RegistryMNT {
    ...
    function registerDNS(
        ...
        //Fee 0.005 * 5000 = 25MNT
        //30d, 50MNT, with fee 75MNT
        //90d, 125MNT, with fee 150MNT
        //365d, 250MNT, with fee 275MNT
        //730d, 500MNT, with fee 525MNT

        // if(network==1){
            if (expiryPrice == 1) {
                _expiryDate = block.timestamp + 2592000; // 30 days
                _price = 75 * 10**18; // 0.01 ether in wei //0.015
            } else if (expiryPrice == 2) {
                _expiryDate = block.timestamp + 7776000; // 90 days
                _price = 150 * 10**18; // 0.025 ether in wei //0.03
            } else if (expiryPrice == 3) {
                _expiryDate = block.timestamp + 31536000; // 365 days
                _price = 275 * 10**18; // 0.05 ether in wei //0.055
            } else if (expiryPrice == 4) {
                _expiryDate = block.timestamp + 63072000; // 730 days
                _price = 525 * 10**18; // 0.1 ether in wei //0.105
            } else {
